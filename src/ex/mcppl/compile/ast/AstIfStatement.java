    ...
    public ByteCode eval(Element e) throws VMException {
        ...
        if(!isbool){
            if(td.getData().equals("true")) {
                ...
                return new JneByteCode(new GroupByteCode(bol),new GroupByteCode(bc));
            }else if(td.getData().equals("false")){
                return new NolByteCode();
            }else throw new VMException("Cannot use keywords in Boolean expressions in IF statements.", e.getPlayer());
        }else if(isbool){
            return new JneByteCode(new GroupByteCode(BoolExpression.calculate(e,BoolExpression.parseBoolExpr(bool))),new GroupByteCode(bc));
        }else throw new VMException("Cannot in if statement boolean use key.",e.getPlayer());
    }
}
